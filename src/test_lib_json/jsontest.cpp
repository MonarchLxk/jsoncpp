...
TestResult &TestResult::addFailure(const char *file, unsigned int line, const char *expr)
{
    ...
    for (; lastNode != nullptr; lastNode = lastNode->next_) {
        if (lastNode->id_ > lastUsedPredicateId_) { // new PredicateContext
            ...
            addFailureInfo(lastNode->file_, lastNode->line_, lastNode->expr_, nestingLevel);
...
void TestResult::addFailureInfo(const char *file, unsigned int line, const char *expr, unsigned int nestingLevel)
{
    Failure failure;
    failure.file_ = file;
    failure.line_ = line;
...
void Runner::runTestAt(size_t index, TestResult &result) const
{
    ...
    try {
        ...
        result.addFailure(__FILE__, __LINE__, "Unexpected exception caught:") << e.what();
    }
#endif // if JSON_USE_EXCEPTION
    delete test;
...
TestResult &checkStringEqual(TestResult &result, const Json::String &expected, const Json::String &actual,
